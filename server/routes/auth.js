...
router.post("/signup", async (req, res, next) => {
  ...
  try {
    ...
    let accessToken = generateAccessToken(userData.email);
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    let accessToken = generateAccessToken(user.email);

    // Optionally handle "remember me"
    if (rememberMe) {
      const refreshToken = await generateAndStoreRefreshToken(user.email);
...
router.post("/logout", async (req, res) => {
  ...
  try {
    ...
    console.log("[!SQL!]Error removing refresh token " + err);
    return res.status(500).json({
      message: "Deleting refresh token error!",
      status: 500,
      errors: [],
    });
  }
